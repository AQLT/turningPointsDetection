...
En termes de déphasage dans la détection des points de retournement, c'est le filtre $b_{q,G}$ qui semble donner les meilleurs résultats quelle que soit la variabilité de la série (graphique \@ref(fig:graphstpsimul)), avec des performances légèrement meilleurs que celles obtenues en prolongeant la série grâce à un modèle ARIMA.
...
rev_table  %>%
  ...
  kable_styling(latex_options=c(#"striped",  
    "hold_position")) %>% 
  pack_rows(index = c("$MAE_{fe}(q) = \\mathbb E\\\\left[\\\\left|(y_{t|t+q} -  y_{t|last})/y_{t|last}\\\\right|\\\\right]$"=8,
...
Les résultats des différentes estimations sont tracées dans les graphiques \@ref(fig:retailxlp) à \@ref(fig:retailxfst2). 
Pour l'approche FST, on ne retient que les filtres obtenus avec les poids $\begin{pmatrix}\alpha&\beta&\gamma\end{pmatrix} = \begin{pmatrix}0,05 &0,00&0,95\end{pmatrix}$, $\begin{pmatrix}\alpha&\beta&\gamma\end{pmatrix} = \begin{pmatrix}0,00 &0,05&0,95\end{pmatrix},\, \begin{pmatrix}0,05 &0,05&0,90\end{pmatrix}$ ou $\begin{pmatrix}0 &0&1\end{pmatrix}$ et en préservant les tendances de degré 2 ou 3. 
...
```{r retailxfst1, echo=FALSE, out.width="90%", fig.cap="Estimations successives de la tendance-cycle des ventes au détail aux États-Unis avec certains poids de la méthode FST."}
img <- sprintf("img/nber/%s_%s.%s",tolower(series),
               "fstp1",
               fig.ext)
knitr::include_graphics(img)
...
```{r retailxfst2, echo=FALSE, out.width="90%", fig.cap="Estimations successives de la tendance-cycle des ventes au détail aux États-Unis avec certains poids de la méthode FST."}
img <- sprintf("img/nber/%s_%s.%s",tolower(series),
               "fstp2",
               fig.ext)
knitr::include_graphics(img)
...
En revanche, les filtres $b_{q,\Gamma}$ et $b_{q,G}$ donnent des estimations intermédiaire proches de l'estimation finale, tout en minimisant le déphasage.

La qualité des estimations intermédiaires peut également être analysée grâces aux prévisions implicites des différentes méthodes. Pour rappel, il s'agit des prévisions de la série brute qui, en appliquant le filtre symétrique de Henderson sur la série prolongée, donne les mêmes estimations que les moyennes mobiles asymétriques. L'annexe \@ref(an-implicitforecasts) rassemble les différentes prévisions implicites des filtres étudiés dans cette section. Les prévisions implicites des filtres polynomiaux autres que LC, des filtres FST et du filtre $b_{q,\varphi}$ sont très peu plausibles et très éloignés des valeurs futures.
