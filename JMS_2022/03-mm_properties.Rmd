...
Cette section présente les définitions et les propriétés des moyennes mobiles utiles pour comprendre les méthodes présentées dans les prochaines sections. Pour plus de détails sur les moyennes mobiles, voir par exemple @ch12HBSA.
...
Cette méthode semble remonter à @deforest1877adjustment qui suggèrent également de modéliser en fin de période une tendance polynomiale de degré au plus trois :
...
Pour les filtres asymétriques, la majorité des critères proviennent de ceux définis par @ch15HBSA et @trilemmaWMR2019 pour construire les filtres asymétriques. Ils sont résumés dans le tableau \@ref(tab:QC) et calculables avec la fonction fonction `rjdfilters::diagnostic_matrix`.
