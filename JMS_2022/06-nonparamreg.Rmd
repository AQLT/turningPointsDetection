# Régression non paramétrique et régression polynomiale locale {#sec-lppfilters}
...
Le noyau trapézoïdal est implémenté dans `rjdfilters` car il permet de calculer les moyennes mobiles utilisées dans l'algorithme X-13ARIMA pour l'extraction des composantes saisonnières. 
...
Les estimations de la tendance-cycle sont en théorie sans bi, même si les estimations sont sans biais, c'est au coût d'une plus grande variance dans les estimations. 
...
Ces filtres asymétriques dépendent du rapport $\delta_1/\sigma$, qui est lié à l'I-C ratio $R=\frac{\bar{I}}{\bar{C}}=\frac{\sum\lvert I_t-I_{t-1}\rvert}{\sum\lvert C_t-C_{t-1}\rvert}$ ($\delta_1/\sigma=2/(R\sqrt{\pi})$), qui est notamment utilisé dans X-13ARIMA pour déterminer la longueur du filtre d'Henderson^[
...
Il est en revanche possible de généraliser d'avantage la modélisation en ajoutant le critère de *timeliness* définit par @ch15HBSA dans l'équation \@ref(eq:lppasym). 
C'est ce qui a été proposé par Jean Palate, puis codé en Java^[https://github.com/palatej/jdemetra-core.] et intégré dans `rjdfilters`.
...
Cette extension permet donc de retrouver tous les filtres symétriques et asymétriques présentés dans la section précédente mais généralise également l'approche de @GrayThomson1996 présentée dans la section \@ref(subsec-graythomson). Elle est proche de l'approche FRST.
...
$$
Dans `rjdfilters` $h$ peut être fixée à la densité spectrale d'un bruit blanc ($h_{WN}(x)=1$, comme c'est le cas dans @dagumbianconcini2008) ou d'une marche aléatoire ($h_{RW}(x)=\frac{1}{2(1-\cos(x))}$).
...
Ainsi, la valeur optimal retenue par défaut par `rjdfilters` produit des discontinuités dans l'estimation de la tendance-cycle.  
...
Par cohérence, simplicité, nous utiliserons dans cet article les valeurs optimales présentées dans @dagumbianconcini2015new.
...
# saveRDS(data_bw, "data/bw_optimal.RDS")
data_bw <- readRDS("data/bw_optimal.RDS")
