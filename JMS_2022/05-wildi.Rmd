...
Les quatre critères $A_w$, $T_w$, $S_w$ et $R_w$ étant des cas particuliers du critère $J$ défini dans l'équation \@ref(eq:theoriegen1), cette méthode s'inscrit dans le cadre de la théorie générale définie dans la section \@ref(subsec-theoriegen).
En effet, en notant :
$$
\begin{cases}
...
C'est ce qui est implémenté dans la fonction `rjdfilters::dfa_filter()`.
...
**`r fa_r_project`** : *package* MDFA https://github.com/wiaidp/MDFA ou `rjdfilters::dfa_filter()` (version simplifiée).
::::
