...
all_tp <- merge(readRDS("results/compile_tp/troughs_fst.RDS"),
                readRDS("results/compile_tp/peaks_fst.RDS"),
                by=c("series","degree", "weight"))
all_tp <- select_var(all_tp)

all_rev_fe <- select_series(readRDS("results/compile_revisions/fst_fe_rev.RDS"))
all_rev_ce <- select_series(readRDS("results/compile_revisions/fst_ce_rev.RDS"))
