...
if(!dir.exists("results_nber/lp"))
  dir.create("results_nber/lp")
library(rjdfilters)
library(AQLThesis)
library("future")
plan(multisession)
...
for(kernel in list_kernel){
  ...
  for (method in c("LC","QL", "CQ", "DAF")){
    ...
    for(s in list_series){
      ...
      nom_f_s <- sprintf("results_nber/lp/%s_%s_%s.RDS",
                         name_file,
                         tolower(kernel), tolower(method))
      nom_f_s_tp <- 
        sprintf("results_nber/lp/%s_%s_%s_tp.RDS",
                name_file,
                tolower(kernel), tolower(method))
      
      nom_f_s_rev_fe <- sprintf("results_nber/lp/%s_%s_%s_fe_rev.RDS",
                                name_file,
                                tolower(kernel), tolower(method))
      nom_f_s_rev_ce <- sprintf("results_nber/lp/%s_%s_%s_ce_rev.RDS",
                                name_file,
                                tolower(kernel), tolower(method))
