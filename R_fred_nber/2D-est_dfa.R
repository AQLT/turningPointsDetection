...
if(!dir.exists("results_nber/dfa"))
  dir.create("results_nber/dfa")
library(rjdfilters)
library(AQLThesis)
library("future")
plan(multisession)
...
for(degree in list_degree){
  ...
  if(!dir.exists(sprintf("results_nber/dfa/dfa%i",degree)))
    dir.create(sprintf("results_nber/dfa/dfa%i",degree))
  for(i in seq_along(all_coefs)){
    ...
    for(s in list_series){
      ...
      nom_f_s <- sprintf("results_nber/dfa/dfa%i/%s_degree%s_weight%i.RDS",
                         degree,
                         name_file,
                         degree,
                         i
      )
      nom_f_s_tp <- sprintf("results_nber/dfa/dfa%i/%s_degree%s_weight%i_tp.RDS",
                            degree,
                            name_file,
                            degree,
                            i
      ...
      nom_f_s_rev_fe <- sprintf("results_nber/dfa/dfa%i/%s_degree%s_weight%i_fe_rev.RDS",
                                degree,
                                name_file,
                                degree,
                                i
      )
      nom_f_s_rev_ce <- sprintf("results_nber/dfa/dfa%i/%s_degree%s_weight%i_ce_rev.RDS",
                                degree,
                                name_file,
                                degree,
                                i
