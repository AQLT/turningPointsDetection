library(AQLThesis)
if(!dir.exists("results_simul/compile_revisions"))
  dir.create("results_simul/compile_revisions")
if(!dir.exists("results_simul/compile_tp"))
  dir.create("results_simul/compile_tp")
if(!dir.exists("results_simul/compile_tp_norev"))
  dir.create("results_simul/compile_tp_norev")
if(!dir.exists("results_nber/compile_revisions"))
  dir.create("results_nber/compile_tp")
if(!dir.exists("results_nber/compile_tp"))
  dir.create("results_nber/compile_tp")
if(!dir.exists("results_nber/compile_tp_norev"))
  dir.create("results_nber/compile_tp_norev")

tp = readRDS("data_simul/tp_simul1.RDS")
suffix = "_fe_rev"
for(suffix in c("_fe_rev", "_ce_rev")){
  
  all_files <- list.files("results_simul/localic_lp/",pattern = suffix,full.names = TRUE)
  
  all_rev <- lapply(seq_along(all_files), function(i){
    print(i)
    f = all_files[i]
    data = readRDS(f)
    data = summary_revisions(data,                    
                             peaks = tp$downturn,
                             troughs = tp$upturn)
    
    full_names <- gsub(sprintf("%s.RDS$", suffix), "", basename(f))
    split <- strsplit(full_names, "_")
    split <- strsplit(full_names, "_")
    series <- sapply(split, `[`, 1)
    method <- sapply(split, `[`, 2)
    h <- sapply(split, `[`, 3)
    degree <- sapply(split, `[`, 4)
    degree[is.na(degree)] <- "d3"
    
    data$series <- series
    data$kernel <- "henderson"
    data$method <- method
    data$h <- h
    data$degree <- degree
    data
  })
  all_rev = do.call(rbind, all_rev)
  
  saveRDS(all_rev, sprintf("results_simul/compile_revisions/lp_localic%s.RDS", suffix))
}

all_files <- list.files("results_simul/localic_lp/",pattern = "_tp",full.names = TRUE)
all_tp_lp <- lapply(seq_along(all_files), function(i){
  print(i)
  f = all_files[i]
  compute_time_lag(readRDS(f),
                   peaks = tp$downturn,
                   troughs = tp$upturn)
})

full_names <- gsub("_tp.RDS$", "", basename(all_files))
split <- strsplit(full_names, "_")
series <- sapply(split, `[`, 1)
method <- sapply(split, `[`, 2)
h <- sapply(split, `[`, 3)
degree <- sapply(split, `[`, 4)
degree[is.na(degree)] <- "d3"
all_t <- data.frame(t(sapply(all_tp_lp,`[[`,"troughs")),
                    series, kernel = "henderson", method, h, degree)
all_p <- data.frame(t(sapply(all_tp_lp,`[[`,"peaks")),
                    series, kernel = "henderson", method, h, degree)
rownames(all_t) <- rownames(all_p) <- full_names

saveRDS(all_t, "results_simul/compile_tp/troughs_lp_localic.RDS")
saveRDS(all_p, "results_simul/compile_tp/peaks_lp_localic.RDS")

all_tp_lp <- lapply(seq_along(all_files), function(i){
  print(i)
  f = all_files[i]
  compute_time_lag(readRDS(f),
                   peaks = tp$downturn,
                   troughs = tp$upturn,
                   type = "no_revisions")
})

full_names <- gsub("_tp.RDS$", "", basename(all_files))
split <- strsplit(full_names, "_")
series <- sapply(split, `[`, 1)
method <- sapply(split, `[`, 2)
h <- sapply(split, `[`, 3)
degree <- sapply(split, `[`, 4)
degree[is.na(degree)] <- "d3"

all_t <- data.frame(t(sapply(all_tp_lp,`[[`,"troughs")),
                    series, method, h, degree)
all_p <- data.frame(t(sapply(all_tp_lp,`[[`,"peaks")),
                    series, method, h, degree)
rownames(all_t) <- rownames(all_p) <- full_names

saveRDS(all_t, "results_simul/compile_tp_norev/troughs_lp_localic.RDS")
saveRDS(all_p, "results_simul/compile_tp_norev/peaks_lp_localic.RDS")

#####################################################

for(suffix in c("_fe_rev", "_ce_rev")){
  all_files <- list.files("results_nber/localic_lp/",pattern = suffix,full.names = TRUE)
  
  all_rev <- lapply(seq_along(all_files), function(i){
    print(i)
    f = all_files[i]
    data = readRDS(f)
    data = summary_revisions(data,                    
                             peaks = nber_tp_m[,"Peak"],
                             troughs = nber_tp_m[,"Trough"])
    
    full_names <- gsub(sprintf("%s.RDS$", suffix), "", basename(f))
    split <- strsplit(full_names, "_")
    split <- strsplit(full_names, "_")
    series <- sapply(split, `[`, 1)
    method <- sapply(split, `[`, 2)
    h <- sapply(split, `[`, 3)
    degree <- sapply(split, `[`, 4)
    degree[is.na(degree)] <- "d3"
    
    data$series <- series
    data$kernel <- "henderson"
    data$method <- method
    data$h <- h
    data$degree <- degree
    data
  })
  all_rev = do.call(rbind, all_rev)
  
  saveRDS(all_rev, sprintf("results_nber/compile_revisions/lp_localic%s.RDS", suffix))
}

all_files <- list.files("results_nber/localic_lp/",pattern = "_tp",full.names = TRUE)
all_tp_lp <- lapply(seq_along(all_files), function(i){
  print(i)
  f = all_files[i]
  compute_time_lag(readRDS(f),                    
                   peaks = nber_tp_m[,"Peak"],
                   troughs = nber_tp_m[,"Trough"])
})

full_names <- gsub("_tp.RDS$", "", basename(all_files))
split <- strsplit(full_names, "_")
series <- sapply(split, `[`, 1)
method <- sapply(split, `[`, 2)
h <- sapply(split, `[`, 3)
degree <- sapply(split, `[`, 4)
degree[is.na(degree)] <- "d3"
all_t <- data.frame(t(sapply(all_tp_lp,`[[`,"troughs")),
                    series, kernel = "henderson", method, h, degree)
all_p <- data.frame(t(sapply(all_tp_lp,`[[`,"peaks")),
                    series, kernel = "henderson", method, h, degree)
rownames(all_t) <- rownames(all_p) <- full_names

saveRDS(all_t, "results_nber/compile_tp/troughs_lp_localic.RDS")
saveRDS(all_p, "results_nber/compile_tp/peaks_lp_localic.RDS")

all_tp_lp_localic <- lapply(seq_along(all_files), function(i){
  print(i)
  f = all_files[i]
  compute_time_lag(readRDS(f),                    
                   peaks = nber_tp_m[,"Peak"],
                   troughs = nber_tp_m[,"Trough"],
                   type = "no_revisions")
})

full_names <- gsub("_tp.RDS$", "", basename(all_files))
split <- strsplit(full_names, "_")
series <- sapply(split, `[`, 1)
method <- sapply(split, `[`, 2)
h <- sapply(split, `[`, 3)
degree <- sapply(split, `[`, 4)
degree[is.na(degree)] <- "d3"

all_t <- data.frame(t(sapply(all_tp_lp,`[[`,"troughs")),
                    series, method, h, degree)
all_p <- data.frame(t(sapply(all_tp_lp,`[[`,"peaks")),
                    series, method, h, degree)
rownames(all_t) <- rownames(all_p) <- full_names

saveRDS(all_t, "results_nber/compile_tp_norev/troughs_lp_localic.RDS")
saveRDS(all_p, "results_nber/compile_tp_norev/peaks_lp_localic.RDS")


